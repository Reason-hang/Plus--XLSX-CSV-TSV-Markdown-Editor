...
- 风险等级：低。
- 背景：真实 IDE 截图显示 H1–H6 每级仅相差 10px，中文长标题下目录近似左对齐，当前活动项还会因负外边距左移。
- 候选方案：仅加大缩进；新增用户配置；重构为可折叠语义树。
- 决策：保持既有标题解析、点击跳转和滚动同步，只在默认 CSS 中使用 20px 级距、低对比度分叉引导、块级可换行链接和原位高亮；不新增设置项、不修改 Markdown 文档、不重构为折叠树。
- 原因：260px 目录面板采用正文列表的 28–34px 级距会显著压缩中文标题宽度；20px 能建立可扫描的层级，同时控制改动面和回归风险。
- 验证方式：类型检查、Lint、构建、本地补丁验证、主题与文档验证；新 VSIX 安装后在三个 IDE 重载窗口进行人工验收。
- 残余风险：外置主题可能覆盖默认目录 CSS；运行时计算样式和实际窗口视觉仍需以安装 `.15` 后的验收为准。
