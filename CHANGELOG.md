...
## v1.9.98-local.16 - Markdown 分栏编辑滚动事务修复

- 分栏编辑在固定/非固定工具栏两种状态下统一使用编辑区与预览区内部滚动；`#content` 不再在分栏状态承担长文档滚动。
- 每次实时预览重绘均使用递增 `renderEpoch`；旧批次的图片、Mermaid 和后续布局工作不会再参与当前页面的同步测量。
- 程序性重绘期间暂时隔离双向同步滚动，避免预览位置恢复反向写回左侧编辑器。
- 同步滚动关闭时不再构建无用途的行映射；保留行高缓存以支持编辑器既有跳转行为。
- 新增受 schema 校验的 `XLSX Viewer Markdown Scroll Diagnostics` 输出通道，记录输入、渲染和四层滚动容器位置，供真实 IDE 验收定位。
- VSIX 版本升级为 `1.9.98-local.16`。

## v1.9.98-local.15 - Markdown 左侧大纲层级可读性修复

- 左侧 Markdown 大纲 H1–H6 的固定级距从 10px 提升至 20px，使深层标题不再近似左对齐。
- 大纲链接改为整行高亮、可在窄面板内换行；活动项使用内部强调条，不再因负外边距向左跳位。
- 为 H2–H6 加入低对比度分叉引导，并添加静态回归断言，防止层级缩进和原位高亮退化。
- VSIX 版本升级为 `1.9.98-local.15`。

## v1.9.98-local.14 - 核心界面汉化简体中文 V3版本修复

- 回溯确认 `.11` 的 Markdown 固定工具栏重构，而非 `.12` 的文案汉化，是分栏编辑和预览编辑出现内容空白、页面异常滚动的结构性根因。
- Markdown 现使用独立 `#markdownToolbarHost`：主工具栏与格式工具栏在同一个 Flex 头部内自然占高，内容区只使用剩余视口，不再动态搬运格式栏、不再使用共享固定头部的异步高度测量。
- 保留无滚动自动聚焦；表格 Webview 继续使用原有共享工具栏布局，不受本次 Markdown 修复影响。
- 增加防止 Markdown 恢复使用共享 fixed-header 重排的静态回归断言。
- VSIX 版本升级为 `1.9.98-local.14`。
